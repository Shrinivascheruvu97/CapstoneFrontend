...
import { Dropdown, DropdownButton, Modal, Button,Form,Table } from 'react-bootstrap';
  ...
  <table className='table table-striped table-bordered table-hover'>
    ...
    {votingData.map((data, index) => (
        <tr key={index}>
          ...
          <td>{data.president_id}</td>
          <td>{data.president_name}</td>
          <td>{data.secretary_id}</td>
          <td>{data.secretary_name}</td>
          <td>{data.treasurer_id}</td>
          <td>{data.treasurer_name}</td>
        </tr>
      ))}
    </tbody>
  </table>
...
//   const chartLabels = resultsData.map((data) => data.member_name);
//   const chartDataValues = 
//   Data.map((data) => data.votes);
